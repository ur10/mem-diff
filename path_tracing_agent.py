...
class MultiPushEnv(gym.Env):
    ...
    def __init__(self,            
        ...
        self.sim_hz = 100
        self.red_done = False
        self.goal_poses = [(50,200), (250,100), (450, 200)]
        self.visit_order = [0,2,0,2,1]
        self.current_visit_idx = 0
        self.goal_poses_dict = {0:(50,200), 2:(250,200), 1:(450,200)}
        ...
        self.legacy = legacy
        self.starting_circle_required = False
        ...
        self.box_shape = pymunk.Poly(self.current_box, [(0,  -30),
                                                    ( -30, 30),
                                                    (30, 30)])
        self.triangle_shape = pymunk.Poly(self.current_box, [(0, -30),
                                                    (-30, 30),
                                                    (30, 30)])
    def reset(self):
    ...
    def _get_goal_pose_body(self, pose, body_type="box"):
        mass = 1
        if body_type == "box":
            ...
            inertia = pymunk.moment_for_poly(mass, [(pose[0], pose[1] - 30),
                                                    (pose[0] - 30, pose[1] + 30),
                                                    (pose[0] + 30, pose[1] + 30)],
                                                    )  
            body = pymunk.Body(mass, inertia)
            ...
            # Add a shape to the body
            shape = pymunk.Poly(body, [(pose[0], pose[1] - 30),
                                                    (pose[0] - 30, pose[1] + 30),
                                                    (pose[0] + 30, pose[1] + 30)])  # Create a triangle shape
    ...
    def render_frame(self,mode):
        ...
        goal_posese = [self.goal_poses[0], self.goal_poses[2]]
        # Draw the 3 goal poses.
        for i, goal_pose in enumerate(goal_posese):
            # print(goal_pose)
        ...
        pygame.draw.polygon(canvas, self.box_color_dict[f"box{self.signal_idx+1}"],
                                [(self.goal_poses[1][0], 
                                  self.goal_poses[1][1] - 30),
                                 (self.goal_poses[1][0] - 30, 
                                  self.goal_poses[1][1] + 30),
                                 (self.goal_poses[1][0] + 30, 
                                  self.goal_poses[1][1] + 30)])
    ...
    def check_agent_intersections(self, frame_count = 0):
        for i, goal_pose in enumerate(self.goal_poses):
            agent_geom,_ = pymunk_to_shapely(self.agent, self.agent.shapes)
            
            if i == 1:
                goal_body = self._get_goal_pose_body([self.goal_poses[1][0], self.goal_poses[1][1]], body_type="triangle")
                
                goal_geom,_ = pymunk_to_shapely(goal_body, [self.triangle_shape])
            else:
                goal_body = self._get_goal_pose_body([self.goal_poses[i][0], self.goal_poses[i][1]])
                goal_geom,_ = pymunk_to_shapely(goal_body, [self.box_shape])

            # print(goal_geom, self.current_box.shapes)
            intersection_area = goal_geom.intersection(agent_geom).area
            goal_area = goal_geom.area
            coverage = intersection_area / goal_area

            if coverage > 0.2:
                required_visit = self.visit_order[self.current_visit_idx]
                current_visit = i
                if required_visit == current_visit:
                    print(f"Signal {i} visited correctly.")
                    return True
                else:
                    # print(f"Signal {i} visited incorrectly.")
                    return False

    def check_starting_circle(self):
        agent_geom,_ = pymunk_to_shapely(self.agent, self.agent.shapes)
        center = Vec2d(200, 450)
        circle = sg.Point(center.x, center.y).buffer(self.signal_circle_radius)
        intersection_area = circle.intersection(agent_geom).area
        circle_area = circle.area
        coverage = intersection_area / circle_area
        if coverage > 0.5:
            print("Returned to starting circle.")
            return True
        else:
            return False
    ...
    def step(self, action):
        ...
        # triangle_goal_body = self._get_goal_pose_body([self.goal_poses[1][0], self.goal_poses[1][1]], body_type="triangle")
        # # print(goal_body.shapes)
        # goal_geom,_ = pymunk_to_shapely(triangle_goal_body, self.current_box.shapes)
        ...
        # goal_area = goal_geom.area
        # coverage = intersection_area / goal_area
        # print(coverage, goal_area)
        reward = 0
        if self.starting_circle_required:
            starting_circle_success = self.check_starting_circle()
            if starting_circle_success:
                print("Starting circle requirement fulfilled.")
                self.starting_circle_required = False
                self.frame_count = 0
                self.current_visit_idx = 0
            return reward, False, False, self._get_info()

        visit_success = self.check_agent_intersections()

        if visit_success:
            print("Visiting correct signal.")
            self.current_visit_idx += 1
            self.frame_count = 0
        if self.current_visit_idx >= len(self.visit_order):
            print("All signals visited successfully!")
            reward = 1
    
        if self.current_visit_idx > 0 and not visit_success:
            if self.frame_count < 32:
                self.frame_count += 1
            else:
                # self.current_visit_idx = 0
                # self.frame_count = 0
                self.starting_circle_required = True
                print("Visited wrong signal, go back to the starting circle.")
    
        # print(f"The current coverage is {coverage}")    
        # reward = np.clip(coverage / self.success_threshold, 0, 1)
        # reward = 1 if visit_success else 0
